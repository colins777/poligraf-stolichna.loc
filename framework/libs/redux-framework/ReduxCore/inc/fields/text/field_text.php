	...
	class ReduxFramework_text {
		...
		function render() {
			...
			if ( isset( $this->field['options'] ) && ! empty( $this->field['options'] ) ) {
				...
				foreach ( $this->field['options'] as $k => $v ) {
					...
					echo '<input ' . $qtip_title . $qtip_text . 'type="' .( isset( $this->field['custom_type'] ) ? $this->field['custom_type'] : 'text' ). '" id="' . $this->field['id'] . '-text-' . $k . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '[' . $k . ']' . '" ' . $placeholder . 'value="' . esc_attr( $this->value[ $k ] ) . '" class="regular-text ' . $this->field['class'] . '"' . $readonly . $autocomplete . ' /><br />';
					echo '</div>';
				}
				//foreach
			} else {
				$placeholder = ( isset( $this->field['placeholder'] ) && ! is_array( $this->field['placeholder'] ) ) ? ' placeholder="' . esc_attr( $this->field['placeholder'] ) . '" ' : '';
				echo '<input ' . $qtip_title . $qtip_text . 'type="' . ( isset( $this->field['custom_type'] ) ? $this->field['custom_type'] : 'text' ). '" id="' . $this->field['id'] . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" ' . $placeholder . 'value="' . esc_attr( $this->value ) . '" class="regular-text ' . $this->field['class'] . '"' . $readonly . $autocomplete . ' />';
			}
		}
		...
		function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style( 'redux-field-text-css', ReduxFramework::$_url . 'inc/fields/text/field_text.css', array(), time(), 'all' );
			}
		}
	}
}