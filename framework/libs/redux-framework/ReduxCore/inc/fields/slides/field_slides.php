...
if ( ! class_exists( 'ReduxFramework_slides' ) ) {
	...
	class ReduxFramework_slides extends ReduxFramework {
		...
		public function render() {
			...
			/*$o = get_option( 'dtheme_option' );
			unset($o['dtheme_repeat']);
			update_option('dtheme_option', $o);*/
			...
			$defaults = array(
				...
				'content_title' => __( 'Block', 'redux-framework' )
			);
			...
			if ( isset ( $this->value ) && is_array( $this->value ) && ! empty ( $this->value ) ) {
				...
				foreach ( $fields as $field ) {
					...
					echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field['id'] . '"><h3><span class="redux-slides-header">' . $this->field['content_title'] . ' ' . ( $x + 1 ) . '</span></h3><div>';

					$field_option = $field;
					//					print_r($field_option);

					echo '<ul id="' . $this->field['name'] . '-ul" class="redux-slides-list" data-slide="' . $x . '">';
					foreach ( $options as $option ) {
						$value           = ( isset( $field_option[ $option['id'] ] ) ) ? $field_option[ $option['id'] ] : '';
						$option['id']    = $this->field['name'] . "[{$x}][{$option['id']}]";
						$option['name']  = $option['id'];
						$option['class'] = '';

						echo '<li>';
						$this->_field_input( $option, $value );

						$field_class = "ReduxFramework_{$option['type']}";
						$field_class = new $field_class($option, $value, $this);
					...
					echo '<li><input type="hidden" class="slide-sort" name="' . $this->field['name'] . '[' . $x . '][sort]' . $this->field['name_suffix'] . '" id="' . $this->field['id'] . '-sort_' . $x . '" value="' . ( isset( $this->field['name'][ $x ]['sort'] ) ? $this->field['name'][ $x ]['sort'] : 0 ) . '" />';
			...
			if ( $x == 0 ) {
				...
				// Redux::$sections['dtheme_option'] // получить параметры
				...
				foreach ( $options as $key => $option ) {
					$options[ $key ]['id']    = $this->field['name'] . "[0][{$option['id']}]";
					$options[ $key ]['name']  = $this->field['name'] . "[0][{$option['id']}]";
					$options[ $key ]['class'] = '';
				}
		...
		public function enqueue() {
			...
			if ( $this->parent->args['dev_mode'] ) {
				...
				wp_enqueue_style( 'redux-field-slides-css', ReduxFramework::$_url . 'inc/fields/slides/field_slides.css', array(), time(), 'all' );
			}
			...
			wp_enqueue_script( 'redux-field-slides-js', ReduxFramework::$_url . 'inc/fields/slides/field_slides' . Redux_Functions::isMin() . '.js', array(
				'jquery',
				'jquery-ui-core',
				'jquery-ui-accordion',
				'jquery-ui-sortable',
