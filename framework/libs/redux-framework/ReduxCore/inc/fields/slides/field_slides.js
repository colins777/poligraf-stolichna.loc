        ...
        $(selector).each(
            function () {
                ...
                el.find('.redux-slides-add').off('click').click(
                    function () {
                        var newSlide = $(document).find('.redux-slides-accordion-group:last').clone(true);
                        var slideCount1 = Number($(newSlide).find('.redux-slides-list').data('slide')) + 1;


                        $(newSlide).find('input[type="text"], input[type="hidden"], textarea, select').each(function () {
                            if ($(this).attr("name") !== undefined) {
                                $(this).attr("name", jQuery(this).attr("name").replace(/\[\d+\]/, '[' + slideCount1 + ']'));
                            }

                            if ($(this).attr("id") !== undefined) {
                                $(this).attr("id", $(this).attr("id").replace(/\[\d+\]/, '[' + slideCount1 + ']'));
                            }
                        ...
                        $(newSlide).find('.redux-slides-list').data('slide', slideCount1);
